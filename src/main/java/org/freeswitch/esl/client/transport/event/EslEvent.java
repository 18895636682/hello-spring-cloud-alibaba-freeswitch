...
public class EslEvent
{
    ...
    public EslEvent( EslMessage rawMessage, boolean parseCommandReply )
    {
        ...
        //added by zhuzhi 20200305
        else if ( rawMessage.getContentType().equals( Value.TEXT_EVENT_JSON ) )
        {
        	parseJsonBody( rawMessage.getBodyLines() );
        }
    ...
    //added by zhuzhi
    private void parseJsonBody( final List<String> rawBodyLines )
    {
    	if(rawBodyLines.size()>1){
    		log.warn("zhuzhi: raw json body should not more than 1 lines");
    		return;
    	}
    	
    	//认为所有的都为Header
//    	System.err.println("----------"+rawBodyLines.get(0));
    	@SuppressWarnings("unchecked")
		Map<String,String> jsonMap = GsonUtils.fromJson(rawBodyLines.get(0), Map.class);
    	for(String key:jsonMap.keySet()){
    		if(jsonMap.get(key) instanceof String){
    			eventHeaders.put(key, jsonMap.get(key));
    		}
    	}
    }
